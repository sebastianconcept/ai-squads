---
description: Squad Template - Lean Squad Definition Structure
globs:
alwaysApply: false
version: 1.0
encoding: UTF-8
---

# [SQUAD_NAME] Squad

## Overview
[One sentence description of the squad's purpose and focus area]

## Members
- **@agent:director** - Project coordination and strategic oversight
- **@agent:[agent-name]** - [Brief description of role]
- **@agent:[agent-name]** - [Brief description of role]
- **@agent:[agent-name]** - [Brief description of role]

## Team Preferences
- **Technology Focus**: [Primary technologies and frameworks]
- **Architecture**: [Architectural preferences and patterns]
- **Development Style**: [Development approach and methodology]
- **Quality Standards**: [Quality requirements and standards]
- **Workflow**: [Preferred workflow and process patterns]
